...
{  
  ...
  //1.使能定时器3和相关 IO 口时钟
	...
	//GPIO_PinRemapConfig(GPIO_PartialRemap_TIM3, ENABLE); //Timer3部分重映射  TIM3_CH1->PA6  
	 ...
	 //2.初始化 IO 口为复用功能输出
   //设置该引脚为复用输出功能,输出TIM3 CH1的PWM脉冲波形	GPIOA.5
...
void TIM1_PWM_Init(u16 arr,u16 psc){
	...
	TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM1; //选择定时器模式:TIM脉冲宽度调制模式2
 	TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable; //比较输出使能
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High; //输出极性:TIM输出比较极性高
	TIM_OC3Init(TIM1, &TIM_OCInitStructure);  //根据T指定的参数初始化外设TIM3 OC3 
	...
	TIM_OC4PreloadConfig(TIM1, TIM_OCPreload_Enable);  //使能TIM3在CCR4上的预装载寄存器
